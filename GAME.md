...
- **Last updated:** 2026-09-07 by Claude (simulation correctness, PR 1 of 2)
- **Branch / working tree:** `fix/simulation-correctness` off `main`
- **Do this next:** Bailey `scripts/test-unity.sh` on the Mac before merge — Unity is the source of truth. Note this branch also repairs a CS0246 that has been breaking every EditMode run and player build on `main` since PR #97. Then Play soak: fill both stands and watch GT-201 hold off-field rather than park on unbuilt Stand 3; drop crew to 2 and confirm delays name a cause.
