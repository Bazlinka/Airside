...
namespace Airside.Presentation
{
    ...
    {
        public ToastEntry(string message, float shownAt, int repeats = 1)
        {
            Message = message;
            ShownAt = shownAt;
            Repeats = repeats;
    ...
    public sealed class ToastQueue
    {
        ...
        public void Push(string message, float now)
        {
            ...
            {
                _history[0] = new ToastEntry(message, now, _history[0].Repeats + 1);
                return;
            }

            _history.Insert(0, new ToastEntry(message, now));
