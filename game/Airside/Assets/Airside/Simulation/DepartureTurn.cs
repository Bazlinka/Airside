...
namespace Airside.Simulation
{
    ...
    public static class DepartureTurn
    {
        ...
        public const double Runway05Degrees = 50.0;
        public const double Runway23Degrees = 230.0;
        ...
        public static float Blend(AircraftPhase phase, float progress)
        {
            if (phase == AircraftPhase.Takeoff)
                return progress <= 0.45f ? 0f : Smooth01((progress - 0.45f) / 0.55f);
            if (phase == AircraftPhase.Departed)
                return 0.55f + 0.45f * Smooth01(progress);
            return 0f;
        }
            ...
            AircraftPhase phase, float progress)
        {
            ...
            return (float)(-Math.Sin(relative) * 220.0 * blend * blend);
        }
        ...
        internal static double RelativeRadians(RunwayDirection runway, Destination home, Destination destination)
        {
            ...
            var runwayHeading = (runway == RunwayDirection.Runway23 ? Runway23Degrees : Runway05Degrees)
                                * Math.PI / 180.0;
