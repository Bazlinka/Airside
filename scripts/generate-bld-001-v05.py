...
def terminal_v05_meshes() -> dict[str, tuple[np.ndarray, np.ndarray]]:
    ...
    # Left / right panels pitch ~4° toward a low central ridge (practical, not steep).
    meshes["roof_panel_l"] = pitched_panel(-5.2, 4.15, 0.05, 10.6, 0.16, 5.15, 4.0)
    meshes["roof_panel_r"] = pitched_panel(5.2, 4.15, 0.05, 10.6, 0.16, 5.15, -4.0)
    meshes["roof_ridge"] = box(0, 4.42, 0.05, 0.55, 0.16, 5.0)
    meshes["roof_eave_front"] = box(0, 3.98, -2.5, 21.0, 0.1, 0.22)
    meshes["roof_eave_back"] = box(0, 3.98, 2.55, 21.0, 0.1, 0.22)
    meshes["roof_parapet"] = box(0, 4.28, -2.45, 20.6, 0.2, 0.1)
    meshes["roof_parapet_back"] = box(0, 4.28, 2.4, 20.6, 0.2, 0.1)
    meshes["fascia_front"] = box(0, 3.82, -2.5, 20.5, 0.18, 0.12)
    meshes["fascia_back"] = box(0, 3.82, 2.5, 20.5, 0.18, 0.12)
    meshes["soffit_front"] = box(0, 3.58, -2.55, 18.0, 0.08, 0.5)
    meshes["roof_flash_front"] = box(0, 4.22, -2.35, 20.4, 0.06, 0.16)
    meshes["roof_flash_back"] = box(0, 4.22, 2.35, 20.4, 0.06, 0.16)

    # Rooftop plant — composed HVAC volumes (not one slab)
    meshes["roof_plant"] = box(-4.6, 4.65, -0.55, 2.4, 0.55, 1.5)
    meshes["roof_plant_b"] = box(3.6, 4.6, 0.45, 2.0, 0.48, 1.25)
    meshes["roof_plant_c"] = box(0.2, 4.55, -0.2, 1.5, 0.38, 1.05)
    meshes["roof_vent_a"] = cylinder(-2.2, 4.85, 0.7, 0.24, 0.5, axis="y", segments=12)
    meshes["roof_vent_b"] = cylinder(2.3, 4.85, -0.5, 0.24, 0.5, axis="y", segments=12)
    meshes["roof_vent_c"] = cylinder(-5.5, 4.75, 0.9, 0.16, 0.35, axis="y", segments=10)
    meshes["hvac_duct"] = box(-6.2, 4.55, 1.15, 3.2, 0.18, 0.32)
    meshes["hvac_duct_b"] = box(5.0, 4.5, -0.9, 2.4, 0.16, 0.28)
