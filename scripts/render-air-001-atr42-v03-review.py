...
p=ROOT/'game/Airside/Assets/Airside/Art/Models/Aircraft/mdl_atr42_starter_v03.gltf';g=json.loads(p.read_text());b=(p.parent/g['buffers'][0]['uri']).read_bytes()
def arr(i):
...
root = Path(__file__).resolve().parents[1]
board = root / 'docs/art/candidates/air_001_atr42_v03_mesh_review.png'
board.parent.mkdir(parents=True, exist_ok=True)
plt.savefig(str(board), dpi=180)
review = root / 'work/review'
review.mkdir(parents=True, exist_ok=True)
artifacts = Path('/opt/cursor/artifacts')
artifacts.mkdir(parents=True, exist_ok=True)
# Re-render each view individually for walkthrough evidence.
solo = [
    (16, -48, 'three-quarter', None),
    (3, -90, 'profile', ((-11, 12.5), (-3, 3), (0, 7.8))),
    (8, 2, 'nose', ((7.4, 12.35), (-2.2, 2.2), (0, 3.7))),
    (90, -90, 'top', ((-11, 12.5), (-13, 13), (0, 0.1))),
    (14, 138, 'tail', None),
]
for elev, az, tag, limits in solo:
    fig2 = plt.figure(figsize=(8, 5), facecolor='#eef0ec')
    draw(fig2.add_subplot(1, 1, 1, projection='3d'), elev, az, tag, limits)
    plt.tight_layout(pad=0.4)
    for folder in (review, artifacts):
        fig2.savefig(str(folder / f'air-001-atr42-v03-{tag}.png'), dpi=160)
    plt.close(fig2)
print('wrote', board)
print('wrote work/review + /opt/cursor/artifacts multi-angle set')
